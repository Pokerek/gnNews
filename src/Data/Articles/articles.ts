...
export const fetchArticles = async (code: string) => {
   // Generate date in format DD-MM-YYYY from yesterday
   const date = new Date();
   date.setDate(date.getDate() - 1);
   const yesterday = date.toLocaleDateString().split('.').reverse().join('-');

   if (!API_KEY) return articles;
   const url = `https://newsapi.org/v2/${
      code === 'xx'
         ? `everything?language=en&from=${yesterday}&to=${yesterday}&domains=bbc.co.uk&pageSize=100&`
